						...
						for(int i=0;i<browserArray.length;i++) {
							for(int count =0;count<parallelThreadCounter.get(browserArray[i]).size();count++){
								  ...
								  if(oldThreadObjects.get(count)!=null && !oldThreadObjects.get(count).isAlive()){
									    ...
									    //"Thread_"+clientName+"_"+browser ==> Thread_Any Bank USA_Gchrome
									    //Thread_Any Bank USA,Bank of North Carolina_null  -thread name
									    clientExecutionStatus.put(threadName.split("_")[1], exeBrowsersList); //Threadname: Thread_Any Bank USA-null_Gchrome    //clientExecutionStatus: {Any Bank USA-null=[Gchrome], Any Bank USA=null}
...
//			t.setName("Thread_"+client_prgmID+"_"+browser);
			String clientName = sequentialThreadObj.get(browserObjCount).client;
			t.setName("Thread_"+clientName+"_"+browser); //threadName.split("_")[1]
	...
	public List<Thread> invokeClientsinParallel(int threadCount, List<CreateThread> executeThreadObjs,String browserName,List<Thread> oldThreadObjects) throws InterruptedException{
		try{
			...
			t=new Thread(obj);
			log.debug("Thread name before setting name: "+t);
			t.setName("Thread_"+executeThreadObjs.get(threadCount).client+"_"+browser);
