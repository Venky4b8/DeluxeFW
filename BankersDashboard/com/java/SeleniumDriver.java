...
public class SeleniumDriver{ 
	...
	public String auctionBrowser;
	public HashMap<Integer, Integer> attributeID;
//	public String prgmID,executionlog,build_ID;
...
//		this.prgmID=obj.prgmID;
		this.executionlog=obj.executionlog;
		this.startTime=obj.startTime;
//		this.build_ID=obj.build_ID;
	}
	
	SeleniumDriver(String browser,String URL,String Clientname,ArrayList<Integer> Tc,String user1,String password1,String user2,String password2,String executionlog,Long startTime2){
	Browser = browser;
	client = Clientname;
	appurl=URL;
	client=Clientname;
...
//		CreateClient.testCasesCounts.put(client.toUpperCase()+"-"+prgmID+"_"+Browser.toUpperCase(), totalTestCases);
...
//		CreateClient.executionStartTime.put(client.toUpperCase()+"-"+prgmID+"_"+Browser.toUpperCase(), hMap.get("StartTime"));
		hostAddress="//"+InetAddress.getLocalHost().getHostAddress();
		//System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
//		String name = "..\\DestinationRewards\\logs\\Log"+"_"+executionlog+"_"+Browser+"_"+client+"-"+prgmID+"_"+hMap.get("TimeStamp") + ".log";
...
//		log.addAppender(fa);
		...
		if(System.getProperty("user.dir").contains("bin"))
		{
			...
			ProjPathLibpath=ProjPath.substring(0, ProjPath.lastIndexOf("\\"));
		}else {
			ProjPath = System.getProperty("user.dir");
			ProjPathLibpath=ProjPath.substring(0, ProjPath.lastIndexOf("\\"));
			System.out.println( hostAddress+"//"+ProjPath.substring(ProjPathLibpath.lastIndexOf("\\")+1));
			log.info("ProjPath:"+ProjPathLibpath);
		}
		 sharedPath=hostAddress+"//"+ProjPath.substring(ProjPathLibpath.indexOf("\\")+1);
		//sharedPath=hostAddress+"//"+ProjPath.substring(ProjPathLibpath.lastIndexOf("\\")+1);
		CreateClient.sharedPath=sharedPath;

//		logFileName=sharedPath+"\\logs\\Log"+"_"+executionlog+"_"+Browser+"_"+client+"-"+prgmID +"_"+hMap.get("TimeStamp") + ".log";
//		CreateClient.logFiles.put(client.toUpperCase()+"-"+prgmID+"_"+Browser.toUpperCase(), logFileName);
...
//		expxl.exportExcelHeader(Browser,client+"-"+prgmID);
		invokeBrowser();

		log.info(Browser + "is opened");
		try {
			hMap.put("ClientName",client);
//			hMap.put("ProgramID", prgmID);
			hMap.put("browser",Browser);
	        hMap.put("Executionlog",executionlog);
			webdriver.get(appurl);
			webdriver.manage().window().maximize();
			testInitiation(Browser,appurl,client,user1,pwd1,user2,pwd2,executionlog,startTime,hostFound);
			tearDown();
...
//			CreateClient.executionStatusForAllClients.put(client.toUpperCase()+"-"+prgmID, executionStatus);
//			log.info("Execution Completed Stopping the thread");
//			CreateClient.updateExecutionStatus(Browser.toUpperCase(),client.toUpperCase()+"-"+prgmID); 
...
//				CreateClient.updateExecutionStatus(Browser.toUpperCase(),client.toUpperCase()+"-"+prgmID); 
	...
	public void invokeBrowser() {
		...
		try {
			...
			}else if("IE8".equalsIgnoreCase(Browser)||"IE9".equalsIgnoreCase(Browser)||"IE10".equalsIgnoreCase(Browser)||"IE11".equalsIgnoreCase(Browser)) {
			...
			try {
				...
				System.out.println("Browser launching: "+Browser.toUpperCase());
				...
				if("IE9".equalsIgnoreCase(Browser)||"IE10".equalsIgnoreCase(Browser)||"IE11".equalsIgnoreCase(Browser)){
					System.setProperty("webdriver.ie.driver",ProjPathLibpath+"/DestinationRewards/drivers/IEDriverServer.exe");
					capability=DesiredCapabilities.internetExplorer();
					capability.setCapability("browserstack.ie.noFlash", "true");
					capability.setCapability(InternetExplorerDriver.INTRODUCE_FLAKINESS_BY_IGNORING_SECURITY_DOMAINS, true);
					capability.setCapability(CapabilityType.ACCEPT_SSL_CERTS, true);
					...
					System.setProperty("webdriver.chrome.driver",ProjPathLibpath+"/DestinationRewards/drivers/chromedriver.exe");
					capability.setCapability(CapabilityType.ACCEPT_SSL_CERTS, true);
...
//				CreateClient.updateExecutionStatus(Browser.toUpperCase(),client.toUpperCase()+"-"+prgmID); 
	...
	/*'#########################################################################################################
	'Function name		:	invokeAuctionBrowser
	'Description		:	This function is for invoke another browser for auctions parllelly

	'Parameters			:	N/A
	'#########################################################################################################*/

	public void invokeAuctionBrowser() {
		FirefoxProfile profile=new FirefoxProfile();
		try {
			nodeName=getNode(Browser.toLowerCase());			
			String hub=java.net.InetAddress.getLocalHost().getHostAddress().toString().trim();			
			String HubPort = "4444";
			URL grid_url = new URL("http://" +hub+ ":" + HubPort + "/wd/hub");
			System.out.println("Hub ip address is-------" +hub);
			log.debug("Hub ip address is-------" +hub);
			System.out.println(nodeName);
			auctionBrowser = getNode("AUC"+ nodeName);
			if("GCHROME".equalsIgnoreCase(auctionBrowser))
			{
				capability=DesiredCapabilities.chrome();
				capability.setCapability(CapabilityType.ACCEPT_SSL_CERTS, true);
			}
			else if("IE8".equalsIgnoreCase(auctionBrowser)||"IE9".equalsIgnoreCase(auctionBrowser)||"IE10".equalsIgnoreCase(auctionBrowser)||"IE11".equalsIgnoreCase(auctionBrowser)){
				capability=DesiredCapabilities.internetExplorer();		
				capability.setCapability("ignoreZoomSetting", true);
				capability.setCapability("browserstack.ie.noFlash", "true");
				capability.setCapability(InternetExplorerDriver.INTRODUCE_FLAKINESS_BY_IGNORING_SECURITY_DOMAINS, true);
			...
			else if("safari".equalsIgnoreCase(auctionBrowser))
			{
				capability=DesiredCapabilities.safari();			
			}
			else{
				if(!("ff".equalsIgnoreCase(auctionBrowser))){
					log.warn("Invalid Browser Name:"+Browser+" Executing the test cases in default browser FireFox");
					System.out.println(("Invalid Browser Name:"+Browser+" Executing the test cases in default browser FireFox"));
				}
				profile.setAssumeUntrustedCertificateIssuer(false);
			...
			auctionWebdriver=new RemoteWebDriver(grid_url, capability);
			Thread.sleep(1000);
			auctionWebdriver.manage().window().maximize();
			Thread.sleep(1000);
			auctionWebdriver.get(appurl);
			log.info("Executing on  GRID");
			System.out.println("The node to be executed is------"+nodeName);
			log.info("Executing in Grid");
			log.info("The node to be executed is------"+nodeName);
			hostFound = null;			
			HttpCommandExecutor ce = (HttpCommandExecutor) auctionWebdriver.getCommandExecutor();
			String hostName = ce.getAddressOfRemoteServer().getHost();
			System.out.println("Hub IPAddress is---------" +hostName);
			log.info("Hub IPAddress is---------" +hostName);
			int port = ce.getAddressOfRemoteServer().getPort();			
			log.info("PortNumber is---------" +hostName);
			HttpHost host = new HttpHost(hostName, port);			
			log.info("HOST---------" +host);
			DefaultHttpClient client = new DefaultHttpClient();
			URL sessionURL = new URL("http://" + hostName + ":" + port + "/grid/api/testsession?session=" + auctionWebdriver.getSessionId());
			System.out.println("The session url is---------" +sessionURL);
			log.info("The session url is---------" +sessionURL);
			BasicHttpEntityEnclosingRequest r = new BasicHttpEntityEnclosingRequest( "POST", sessionURL.toExternalForm());
			HttpResponse response = client.execute(host, r);
			...
			if ((myURL.getHost() != null) && (myURL.getPort() != -1)) {
				hostFound = myURL.getHost();
				System.out.println("Node ip address" +hostFound);
				log.info("Node ip address" +hostFound);
			}					
		}catch(Exception e) {
			if(!isGridExecution){
				try {
					...
					if(("gchrome".equalsIgnoreCase(Browser))){
						//Launching Firefox as acution browser
						profile.setAssumeUntrustedCertificateIssuer(false);
						capability=DesiredCapabilities.firefox();
						capability.setCapability(FirefoxDriver.PROFILE,profile);
						auctionWebdriver = new FirefoxDriver(capability);
					}else{
						//Launching Chromedriver as auction browser
						System.setProperty("webdriver.chrome.driver",ProjPathLibpath+"/DestinationRewards/drivers/chromedriver.exe");
						...
						auctionWebdriver = new ChromeDriver(co);
					}
					Thread.sleep(1000);
					auctionWebdriver.manage().window().maximize();
					auctionWebdriver.get(appurl);
				}catch(Exception ex) {log.info("Failed to launch auction browser in local" +hostFound);}		
			}			
		}
	}
	...
	public String getNode(String browser)
	{
		...
		String dirpath = System.getProperty("user.dir");
		String projpath =dirpath.substring(0, dirpath.lastIndexOf("\\")) ;
		try {
			FileInputStream in = new FileInputStream(projpath+"/DestinationRewards/properties/Grid.properties");
			gridprops.load(in);
	...
	public void testInitiation(String browser,String appUrl,String client,String user1,
			String password1,String user2,String password2,
			String executionlog,Long lngTestStepExecutionStartTime,String NodeIp) throws Exception {
				...
				if(TestCases.get(i).equals(tdd.getTestCaseID()) && (tdd.getBrowserType().equalsIgnoreCase("COMMON") || tdd.getBrowserType().toUpperCase().indexOf(Browser.toUpperCase()) != -1)&&getClient(tdd.getclientName(),tdd)){
					...
					if(actionType == null){
						...
						if(dataFields!=null && dataFields.substring(0,3).equals("AUC")){
							onAuctionStep=true;
							if(!auctionSetFlag){
								invokeAuctionBrowser();
								Thread.sleep(30000);
								auctionSetFlag=true;
							}
							dataFields = dataFields.substring(3, dataFields.length());
							resultDetails = test.performAction(auctionWebdriver,dataFields,dataValues,actionType, fieldName,browser,user1,password1,user2,password2);
						}else{
							resultDetails = test.performAction(webdriverCopy,dataFields,dataValues, actionType, fieldName,browser,user1,password1,user2,password2);					
					...
					if(!resultDetails.getFlag()){
						...
						if ("FF".equalsIgnoreCase(browser)||"IE8".equalsIgnoreCase(browser) ||"IE9".equalsIgnoreCase(browser)||"IE10".equalsIgnoreCase(browser)||"IE11".equalsIgnoreCase(browser)||"GCHROME".equalsIgnoreCase(browser)||"safari".equalsIgnoreCase(browser)) {
							...
							String reportsPath = ProjPathLibpath+"\\DestinationRewards\\TestReports\\"+strDirectoryName;
							try {
//								htmlReportsPath=hostAddress+"//"+ProjPath.substring(ProjPathLibpath.lastIndexOf("\\")+1)+"\\TestReports\\"+strDirectoryName;
								htmlReportsPath=hostAddress+"//"+ProjPath.substring(ProjPathLibpath.indexOf("\\")+1)+"\\TestReports\\"+strDirectoryName;
							}catch(Exception e) {
						...
						try {
							if ((tdd.getCondition() != null) && (tdd.getCondition().indexOf("IF") != -1) && (tdd.getActionType().toUpperCase() == "VERIFY")) {	
								...
								if (arrCon[1].split(":")[0].toUpperCase().equalsIgnoreCase("NEXT")) {
									System.out.println("Continue next step...");
									log.debug("Continue next step...");
								} else {
									new java.math.BigInteger(arrCon[1].split(":")[0]);
									k = Integer.parseInt(arrCon[1].split(":")[0]);
									System.out.println( "step change to "+ k);
									log.debug( "step change to "+ k);
								}	    					
								k = Integer.parseInt(arrCon[2].split("-")[1])-1;
								System.out.println( "step change to "+ k);
								log.debug( "step change to "+ k);
							}
						}catch (NumberFormatException ex) {
							System.out.println("invalid step number."+arrCon[1].split(":")[0]);
							log.debug("invalid step number."+arrCon[1].split(":")[0]);
						}
				...
				//Loop the steps
				try {
					if (tdd.getCondition() != null && tdd.getCondition().toUpperCase().indexOf("LOOP") != -1&&(!FailedCaseScreenShot.get(tdd.getTestCaseID()).contains("Browser Crashed"))) {
						int from = k+1;
						...
						test.executeSteps(webdriver, from, to , lcnt, i, TestData, resultDetails, result,browser,user1,password1,user2,password2);
					}
				} catch (NumberFormatException ex) {
					System.out.println("invalid step number."+arrCon[0].split(":")[1]);
					log.debug("invalid step number."+arrCon[0].split(":")[1]);
			...
			if (caseFound) {	
				...
				log.info("The results are-------------------" +exportResult);
//				expxl.exportExcelRows(exportResult,browser,client+"-"+prgmID);
...
//				expxl.exportExcelRows(exportResult,browser,client+"-"+prgmID);
	...
	/*	if(browser.equalsIgnoreCase("ff"))			
		{			
			failMap.put("failedCases_ff"+client+"-"+prgmID+""+browser,failedCases+"_"+client+"-"+prgmID+"_"+browser);	
		}
		if(browser.equalsIgnoreCase("IE8"))
		{	
			failMap.put("failedCases_ie8"+client+"-"+prgmID+""+browser,failedCases+"_"+client+"-"+prgmID+"_"+browser);	
		}*/
		if(browser.equalsIgnoreCase("IE9"))
		{	
			failMap.put("failedCases_ie9"+client+"-"+""+""+browser,failedCases+"_"+client+"-"+""+"_"+browser);	
		}
		if(browser.equalsIgnoreCase("IE10"))
		{	
			failMap.put("failedCases_ie10"+client+"-"+""+""+browser,failedCases+"_"+client+"-"+""+"_"+browser);	
		}
		if(browser.equalsIgnoreCase("IE11"))
		{	
			failMap.put("failedCases_ie11"+client+"-"+""+""+browser,failedCases+"_"+client+"-"+""+"_"+browser);	
		}
	/*	if(browser.equalsIgnoreCase("gchrome"))
		{	
			failMap.put("failedCases_gchrome"+client+"-"+prgmID+""+browser,failedCases+"_"+client+"-"+prgmID+"_"+browser);	
		}
		if(browser.equalsIgnoreCase("safari"))
		{	
			failMap.put("failedCases_safari"+client+"-"+prgmID+""+browser,failedCases+"_"+client+"-"+prgmID+"_"+browser);	
		}
*/
	...
	public void getDataSource(){
		...
		try {
			FileInputStream in = new FileInputStream(CreateClient.dirPath+"/properties/dataSource.properties");
			dbProps.load(in);
	...
	/*'#########################################################################################################
	'Function name		:	INVOKESAFARIBROWSER
	'Description		:	Method the to invoke the safari browser
	'						
	'Parameters			:	N/A
	'#########################################################################################################*/
	
	public  synchronized void invokeSafariBrowser() throws UnknownHostException, MalformedURLException, InterruptedException{
		log.debug("In invokeSafariBrowser method");
		String nodeName=getNode(Browser.toLowerCase());	
		...
		webdriver=new RemoteWebDriver(grid_url, capability);	
		webdriver.manage().window().maximize();
		log.debug("In invokeSafariBrowser method browser maximized");
		Thread.sleep(10000);
		try {
			webdriver.get(appurl);
		}catch(Exception e) {
