...
public class CreateClient{
	...
	static String destinationRewardsLOGO="https://employee.drwqa.com/img/logo/dr_logo.jpg";
	...
	public static void main(String[] args) throws SQLException, MessagingException, IOException, InterruptedException {
		...
		String xlsPath= ProjPath+"\\DestinationRewards\\TestInputs\\Config.xls";
		String xlsPath2= ProjPath+"\\DestinationRewards\\TestInputs\\TestData.xls";
	...
	/*	String url1="jdbc:sqlserver://10.120.100.52:61446;databaseName=automation_tests";
		...
		//Connecting the database to find the latest execution log id in the TesttepLog table.
//		try {
//			Class.forName(jdbcDriver1);
//			conn1 = DriverManager.getConnection(url1,userName1,password1);
//			log.info("Connected successfully");
//			String sql="Select Max(ExecutionLog)  from TestStepLog";
//			st5=conn1.createStatement();
//			rs = st5.executeQuery(sql);
//			while(rs.next()){
//				int exlog=rs.getInt(1);
//				exlog = exlog+1;
//				executionlog= Integer.toString(exlog);
//				log.info("The Execution Log ID is-----: "+executionlog);
//				System.out.println("Execution Log ID-----"+executionlog);	
//			}		
//		}catch(Exception e){
...
//			conn1.close();
//		}
		String name = "..\\DestinationRewards\\logs\\Log"+"_"+executionlog+"_"+  dateFormate.format(new Date()) + ".log";
		...
		try {
			FileInputStream inFile = new FileInputStream(dirPath+"/properties/Misc.properties");
		...
		try {	
			...
			if(args.length != 0) {
				...
				try {
					...
					con = DriverManager.getConnection("jdbc:odbc:Driver={Microsoft Excel Driver (*.xls)};DBQ=" +xlsPath+ ";DriverID=22;READONLY=false","","");
					st = con.createStatement();
					query = "Update [Setup$] set \"Test Cases to be Executed\"=" +"'"+args[0]+"'"+""+",\"Browser Type\"=" + "'"+args[1] +"'"+ ", \"Client Name\"=" +"'"+ args[2] +"'"+", \"Execution Type\"="+"'"+args[3]+"'"+ ", \"Build ID\"="+"'"+args[4]+"'";
			...
			Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
			conn = DriverManager.getConnection("jdbc:odbc:Driver={Microsoft Excel Driver (*.xls)};DBQ=" +xlsPath+ ";DriverID=22;READONLY=false","","");
			...
			while(rs.next()){
				...
				for(int i=0;i<tokens.length;i++){							    	
					if(!tokens[i].contains("-"))
						testcases1.add(Integer.parseInt(tokens[i]));
					else{
				...
				log.info("Test Cases Being executed:"+testcases1);
...
//					prgmid=clients[k].split("-")[1];
//					String prgmidArray[]=null;
//					prgmidArray=prgmid.split(":");
//					for(programCount=0;programCount<prgmidArray.length;programCount++) { //Prog ID loop
						/*prgmid=prgmidArray[programCount];
						clientNameArray.add(client+"-"+prgmid); 
						clientExecutionStatus.put(client+"-"+prgmid, null);
						String url="jdbc:sqlserver://10.120.100.52:61446;databaseName=Drcore_test";
						String password = "p*fR3bCE!5Hz3H_90dNvi"; 
						String userName = "auto_app"; 
						String jdbcDriver = "com.microsoft.sqlserver.jdbc.SQLServerDriver"; 
						ArrayList<Integer> testcases2 = new ArrayList<Integer>();

						//Connecting the database to find the attributeID value set for the programID
						try {
...
//							st3 = conn1.createStatement();
							Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
							conn2 = DriverManager.getConnection("jdbc:odbc:Driver={Microsoft Excel Driver (*.xls)};DBQ="+xlsPath2+";DriverID=22;READONLY=false","","");
							int attrbid[] ={554,1511,360,331,312,375,301,302,903,923,980,399,323,949,1524};

							for(int i=0;i<attrbid.length;i++){
								String sql4= "select attributevalue from programattributes where attributeid= "+attrbid[i]+ " and programid="+prgmid+" and attributevalue=1" ;
//								rs4 = st3.executeQuery(sql4);
								int attrvalue;
//								while(rs4.next())
								{
//									attrvalue= rs4.getInt(1);
									int count=1;
//									if(attrvalue==1)
									{
										count =count++;
										String sql5="Select distinct \"Test Case ID\" from [DataSheet$] Where \"Attribute ID\" =" +attrbid[i]+"";
										st4 = conn2.createStatement();
										rs5 = st4.executeQuery(sql5);
										while(rs5.next()) {
											count=count++;
											int tc=rs5.getInt(1);
											attrMap.put(tc,attrbid[i]);
											testcases2.add(tc);
										}
									}
								}
							...
							try {
								ArrayList<Integer> testcasesdump = new ArrayList<Integer>(testcases1);
								testcasesdump.retainAll(testcases2);
								testcases = new ArrayList<Integer>(new HashSet<Integer>(testcasesdump));
...
//						log.info("Program id: " +prgmid);
						
						sql3="Select  *  from [Users$]  where \"Client Name\" = '"+client+"'";
//						sql3="Select  *  from [AppUrls$]  where \"Client Name\" = '"+client+"'";
						st3 = conn.createStatement();
						rs3 = st3.executeQuery(sql3);
						while(rs3.next())
						{
							URL = rs3.getString("Client URL");
//							CreateClient.clientLOGO.put(client.toUpperCase(), rs3.getString("Application Logo URL"));
//							CreateClient.dailyDealCategory.put(client.toUpperCase(), rs3.getString("Daily Deal Category"));
//							CreateClient.clientID.put(client.toUpperCase(), rs3.getString("Client ID"));
							appUrls.add(URL);
							...
							for(int i = 0; i < browserColumn.split(",").length; i++) {
    							...
    							List<HashMap<String,Boolean>> browserList=(clientWiseExecutionStatus.get(clients[k].toUpperCase())==null) ? new ArrayList<HashMap<String,Boolean>>():clientWiseExecutionStatus.get(clients[k].toUpperCase()); 
								 ...
								 //------------ 
//								sql2="Select  *  from [Users$] where \"Client Name\" = '"+client+"-"+prgmid+"'"; 
								 sql2="Select  *  from [Users$] where \"Client Name\" = '"+client+"'";
								st2 = conn.createStatement();
								rs2 = st2.executeQuery(sql2);
								while(rs2.next()){
									if("IE9".equalsIgnoreCase(browserArray[i])){
										loginuser=	rs2.getString("IE8");	
									}
//									else if("IE9".equalsIgnoreCase(browserArray[i])) {
...
//									}
									...
									}else if("safari".equalsIgnoreCase(browserArray[i])){
										loginuser=	rs2.getString("safari");
									}*/
									user1=loginuser.split(",")[0].split(":")[0];
									pwd1=loginuser.split(",")[0].split(":")[1];
									user2=loginuser.split(",")[1].split(":")[0];
									pwd2=loginuser.split(",")[1].split(":")[1];
									System.out.println("Browser Name: " +browserArray[i].toUpperCase()+"\nLogin UserID: " +user1+ "\nLogin Password: " +pwd1+"\nAuction UserID: " +user2+"\nAuction UserPassword: " +pwd2);	
									log.info("Browser Name:" +browserArray[i]+"\nLogin user:" +user1+ "\nLogin Password:" +pwd1+"\nAuction user:" +user2+"\nPassword:" +pwd2);
								}
								CreateThread obj= new CreateThread(browserArray[i],URL,client,testcases1,user1,pwd1,user2,pwd2,executionlog,lngTestStepExecutionStartTime);
								...
								t.setName("Thread_"+client+"-"+""+"_"+browserArray[i]);
								...
								/*if("Parallel".equalsIgnoreCase(executionType)) {	
									...
									t= new Thread(executeThreadObj);
									log.debug("Thread name before setting name: "+t);
									t.setName("Thread_"+client+"-"+prgmid+"_"+browserArray[i]);
									log.debug("Thread name: "+t);
									t.start();
									Thread.sleep(1000);
									threads.add(t); 
...
//			if("Sequential".equalsIgnoreCase(executionType.split(":")[0])) {
				try{	
...
//			}
	...
	public List<Thread> invokeClientInSequential(List<CreateThread> sequentialThreadObj,List<Thread> oldThreadObjects,int browserObjCount,String browser) throws InterruptedException{
		try{
		    ...
		    String client_prgmID = sequentialThreadObj.get(browserObjCount).client+"-"+sequentialThreadObj.get(browserObjCount);
...
//			t.setName("Thread_"+executeThreadObjs.get(threadCount).client+"-"+executeThreadObjs.get(threadCount).prgmID+"_"+browserName);
	...
	public static void updateExecutionStatus(String browserName,String ClientName){
		...
		browseWiseExecutionStatus.get(browserName.toUpperCase()).get(0).put(ClientName.toUpperCase(),true);		
		clientWiseExecutionStatus.get(ClientName.toUpperCase()).get(0).put(browserName.toUpperCase(),true);
	}
...
//	String build_ID;
	...
	CreateThread(String browser,String URL,String Clientname,ArrayList<Integer> Tc,String user1,String password1,String user2,String password2,String executionlog,Long lngTestStepExecutionStartTime){
		client = Clientname;
		browsername=browser;
		appurl=URL;
		client=Clientname;
