...
public class EmailTestReport
{
	...
	public EmailTestReport() {
		try {
			InputStream is= new FileInputStream(CreateClient.ProjPath+"/properties/Email.properties");
	...
	public void postMail_BrowserLevel(
		...
		/*	for(int x=0;x<recipientsNames.length;x++)
			...
			addressTo =new InternetAddress[recipientsNames.length];
			for (int i = 0; i < recipientsNames.length; i++)
			{
				addressTo[i] = new InternetAddress(recipientsNames[i]);
			
				sd.log.info(" Added " + recipientsNames[i]);
			}
